...
class Stylisheve_OrderActionsButtons_Adminhtml_OrderbuttonController extends Mage_Adminhtml_Controller_Action
{
    ...
    {
        $this->loadLayout()->_setActiveMenu("orderactionsbuttons/orderbutton")->_addBreadcrumb(Mage::helper("adminhtml")->__("Order Buttons  Manager"), Mage::helper("adminhtml")->__("Order Buttons Manager"));
        return $this;
    }

    public function indexAction()
    {
        $this->_title($this->__("Order Actions Buttons"));
        $this->_title($this->__("Manager Order Buttons"));
    ...
    public function editAction()
    {
        ...
        $this->_title($this->__("Order Button"));
        $this->_title($this->__("Edit Order Button"));
        ...
        if ($model->getId()) {
            ...
            $this->_addBreadcrumb(Mage::helper("adminhtml")->__("Order Buttons Manager"), Mage::helper("adminhtml")->__("Order Buttons Manager"));
            $this->_addBreadcrumb(Mage::helper("adminhtml")->__("Order Buttons Description"), Mage::helper("adminhtml")->__("Order Buttons Description"));
            ...
            Mage::getSingleton("adminhtml/session")->addError(Mage::helper("orderactionsbuttons")->__("Order Button does not exist."));
            $this->_redirect("*/*/");
        }
    }
    ...
    public function newAction()
    {
        $this->_title($this->__("Order Actions Buttons"));
        $this->_title($this->__("Order Button"));
        $this->_title($this->__("New Order Button"));
    ...
    public function saveAction()
    {
        ...
        if ($post_data) {
            ...
            try {
                ...
                Mage::getSingleton("adminhtml/session")->addSuccess(Mage::helper("adminhtml")->__("Order button was successfully saved"));
    ...
    public function deleteAction()
    {
        if ($this->getRequest()->getParam("id") > 0) {
            try {
                ...
                Mage::getSingleton("adminhtml/session")->addSuccess(Mage::helper("adminhtml")->__("Order button was successfully deleted"));
                $this->_redirect("*/*/");
            } catch (Exception $e) {
                Mage::helper('orderactionsbuttons')->logException('OrderActionsButtons.log',
    ...
    public function massRemoveAction()
    {
        try {
            ...
            Mage::getSingleton("adminhtml/session")->addSuccess(Mage::helper("adminhtml")->__("Order Button(s) was successfully removed"));
        } catch (Exception $e) {
            Mage::helper('orderactionsbuttons')->logException('OrderActionsButtons.log',
    ...
    public function _prepareSelectOrdersQueryForGenerateReport(&$pOrdersModel, $pBtnData, $pRoleId)
    {
        ...
        if ($pBtnData->getCheckWarehouse()) {
            ...
            $pOrdersModel->getSelect()->join(
                ['sfoi' => 'sales_flat_order_item'], 'sfoi.order_id = `main_table`.entity_id',
                ['sfoi.product_type', 'sfoi.name', 'sfoi.sku', 'sfoi.qty_ordered']
            )->join(
                ...
                ['swr' => 'stylisheve_warehouses_roles'], 'FIND_IN_SET(esfoi.preparation_warehouse, swr.warehouse_id)', []
            )->where("swr.role_id = $pRoleId AND sfoi.product_type = 'simple'");
        }//endIF
        if ($pBtnData->getCheckDeliveryDate()) {
            $pOrdersModel->getSelect()->join(
                ['aad' => 'amasty_amdeliverydate_deliverydate'], 'aad.order_id = `main_table`.entity_id', ['date']
    ...
    public function _getReportContentHtml(&$pTableDataArray, $pReportHtmlContent, $pReportNo, $pOrdersCount, $pTableContentType)
    {
        ...
        switch ($pTableContentType):
            case 'items':
                ...
                break;
            case 'orders':
                $reportTable .= '<th>Order ID</th><th>Color</th><th>Total</th><th>City</th><th>Country</th><th>Payment Method</th>' . PHP_EOL;
                break;
        endswitch;
        $reportTable .= '</tr>' . PHP_EOL;
        $reportTable .= '</thead>' . PHP_EOL;
